...
if ( isset( $_POST ) && $_POST ) {
  ...
  $offset = ( $page - 1 ) * $limit;
  $sql = "SELECT id,name,content,address,image,latitude,longitude FROM creative_economy WHERE";
  $sql.= ( ! is_null( $query ) ) ? " name LIKE '%$query%' OR content LIKE '%$query%' OR address LIKE '%$query%'" : " category_id = $category_id";
  $creative_economy = $aeo_db->get_results( "$sql ORDER BY added_datetime DESC LIMIT $offset, $limit" );
  $creative_economy_total = $aeo_db->get_count( $sql );

  if ( $creative_economy ) {
    foreach ( $creative_economy as $item ) {
      $summary = strip_tags( trim( $item['content'] ) );
      $summary = ( strlen( $summary ) > 200 ) ? substr( $summary, 0, 200 ) . ' ...' : $summary;

      if ( $latitude && $longitude )
        $distance = $aeo_db->get_row( "SELECT ( 6373 * acos( cos( radians( $latitude ) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians( $longitude ) ) + sin( radians( $latitude ) ) * sin( radians( latitude ) ) ) ) AS distance FROM creative_economy WHERE id = $item[id]" );
      ...
      $response['items'][] = [
        ...
        'image' => ( $item['image'] ) ? site_url() . '/uploads/creative-economy/' . $item['image'] : ''
      ];

      $response['total_pages'] = ceil( $creative_economy_total / $limit );
    }
  }
} else {
