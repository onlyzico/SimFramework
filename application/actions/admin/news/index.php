...
if ( ! current_user_can( 'admin_news' ) )
...
if ( isset( $_POST['action'] ) && $_POST['action'] && isset( $_POST['csrf_token'] ) && $_POST['csrf_token'] == csrf_token() ) {
  if ( $_POST['action'] == 'delete_selected' && isset( $_POST['ids'] ) && $_POST['ids'] ) {
    ...
    foreach ( $ids as $id ) {
      $data_before = $aeo_db->get_row( "SELECT image FROM news WHERE id = $id" );
      ...
      $aeo_db->delete( 'news', [ 'id' => $id ] );
    }

    create_session( 'admin_news_success_msg', count( $ids ) . ' Berita berhasil dihapus' );
  }
...
if ( get_session( 'admin_news_success_msg' ) ) {
  $success_msg = get_session( 'admin_news_success_msg' );
  delete_session( 'admin_news_success_msg' );
}

/*----------------------------------------------------------------------------*/

$aeo['site_title'].= $aeo['breadcrumb_title'] = $aeo['page_title'] = 'Berita';
...
$aeo['localize_script']['datatable_data_name'] = 'berita';
$aeo['localize_script']['datatables_load_slug'] = 'load-news';
